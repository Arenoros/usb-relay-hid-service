...
static const char* usbErrorMessage(int errCode) {
    ...
    buffer[0] = 0;
    if(errCode != USBHID_ERR_UNKNOWN) { usbhidStrerror_r(errCode, buffer, sizeof(buffer)); }
    if(0 == buffer[0]) { snprintf(buffer, sizeof(buffer), "Unknown error (%d)", errCode); }
    return buffer;
}
...
*/
int USBRL_API usb_relay_init(void) { return 0; }
...
@returns: This function returns 0 on success and -1 on error.
*/
int USBRL_API usb_relay_exit(void) { return 0; }

// Enum function for building list of devices
static int enumfunc(USBDEVHANDLE usbh, void* context) {
    ...
    i = strlen(buffer);
    if(i != strlen(productName) + 1) { goto next; }

    /* the last char of ProductString is number of relays */
    num = (int)(buffer[i - 1]) - (int)'0';
    buffer[i - 1] = 0;

    if(0 != strcmp(buffer, productName)) { goto next; }
...
static int enumOpenfunc(USBDEVHANDLE usbh, void* context) {
    //    static const char vendorName[] = USB_RELAY_VENDOR_NAME;
    static const char productName[] = USB_RELAY_NAME_PREF;
    int err;
    char buffer[128 * sizeof(short)];  // max USB string is 128 UTF-16 chars
...
pusb_relay_device_info_t USBRL_API usb_relay_device_enumerate(void) {
    ...
    ret = usbhidEnumDevices(USB_CFG_VENDOR_ID, USB_CFG_DEVICE_ID, (void*)&ectx, enumfunc);
...
intptr_t USBRL_API usb_relay_device_open_with_serial_number(const char* serial_number, unsigned len) {
    ...
    if(!q) return (intptr_t)0;

    memcpy(q->idstr, serial_number, len);

    ret = usbhidEnumDevices(USB_CFG_VENDOR_ID, USB_CFG_DEVICE_ID, (void*)&ectx, enumOpenfunc);
    if(ret != 0) goto ret_err;  // error during enum
...
intptr_t USBRL_API usb_relay_device_open(struct usb_relay_device_info* device_info) {
    ...
    if((uintptr_t)p->usbh == 0 || (uintptr_t)p->usbh == (uintptr_t)-1) return 0;
    //$$$ validate more
...
void USBRL_API usb_relay_device_close(intptr_t hHandle) {
    ...
    if(0 == hHandle || ((intptr_t)-1) == hHandle) return;
...
int USBRL_API usb_relay_device_open_one_relay_channel(intptr_t hHandle, int index) {
    struct usbrelay_internal_s* p = (struct usbrelay_internal_s*)hHandle;
    if(!p) return 1;
    if(index <= 0 || index > (int)p->urdi.type) return 2;
...
*/
int USBRL_API usb_relay_device_lib_version(void) { return (int)(MY_VERSION); }
...
intptr_t USBRL_API usb_relay_device_next_dev(intptr_t ptr_usb_relay_device_info) {
    if(!ptr_usb_relay_device_info) return 0;
